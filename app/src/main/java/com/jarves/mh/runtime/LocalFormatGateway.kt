...
internal class LocalFormatGateway(
    ...
    // Binding to 127.0.0.1 keeps other devices out, but on-device any process
    // that can reach loopback (in particular anything running inside the same
    // PRoot guest, e.g. a malicious npm postinstall script) could otherwise
    // send requests here and spend the user's provider quota/credits without
    // ever touching apiKey. A random per-session bearer token, known only to
    // this instance and to the Claude Code process we launch (see
    // RuntimeLaunchConfigBuilder passing localToken as an extra env var),
    // closes that gap without adding a second copy of the real provider key.
    val localToken: String = UUID.randomUUID().toString()
    ...
    private fun handle(socket: Socket) {
        ...
        val presentedToken = headers["x-api-key"] ?: headers["authorization"]?.removePrefix("Bearer ")
        if (presentedToken != localToken) {
            // Constant-time-ish reject: don't distinguish "wrong token" from
            // "malformed request" in timing or message detail.
            writeJson(output, 401, errorJson("authentication_error", "Invalid local gateway token"))
            return
        }
