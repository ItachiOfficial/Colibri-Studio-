...
class ClaudeRuntimeBridge(
    ...
    override suspend fun startSession(projectId: String, projectSlug: String, projectKind: ProjectKind, prompt: String, conversationHistory: List<ChatMessage>, provider: ProviderProfile): String = withContext(Dispatchers.IO + NonCancellable) {
        ...
        runCatching {
            ...
            // When a gateway is in play, hand Claude Code a random per-session token
            // instead of the real provider key: the gateway checks that token on
            // every inbound request (see LocalFormatGateway.handle) and only it
            // holds the real key needed to call the upstream OpenAI-compatible
            // provider. That way the real key never sits in the guest process's
            // environment or crosses the loopback socket at all.
            val launch = RuntimeLaunchConfigBuilder.build(
                provider,
                authToken = formatGateway?.localToken ?: secret,
    ...
    /**
     * Only HIGH-risk commands ever produce a `.request` file: the shell hook
     * (permission-hook.sh) auto-allows everything else before it reaches
     * here. Anything seen in this loop is therefore already known to be
     * risky by the same heuristic classifyRisk() uses, and gets a real
     * approval round-trip instead of a silent allow.
     */
    private suspend fun watchPermissionRequests(sessionId: String) {
        ...
        while (kotlin.coroutines.coroutineContext.isActive) {
            bridge.listFiles { file -> file.name.endsWith(".request") }.orEmpty().forEach { file ->
                ...
                if (approvalId in pending.keys || approvalId in seenToolCalls) return@forEach
                runCatching {
                    ...
                    seenToolCalls.add(approvalId)
                    val request = ToolRequest(
                        approvalId = approvalId,
                        sessionId = sessionId,
                        toolName = toolName,
                        explanation = sanitizeForDisplay(explanation),
                        affectedPaths = paths,
                        commandPreview = command?.let(::sanitizeForDisplay),
                        risk = classifyRisk(toolName, command),
                    )
                    pending[approvalId] = PendingPermission(request, response)
                    Log.d("ClaudeBridge", "Awaiting approval $approvalId for $toolName ($paths)")
                    eventBus.emit(RuntimeEvent.ToolRequested(sessionId, request))
                    pushForegroundProgress("Waiting for your approval · $toolName")
                }.onFailure {
                    // Malformed request file: never silently allow. Deny it and move on.
