...
class RuntimeExecutionService : Service() {
    ...
    companion object {
        ...
        fun ensureNotificationChannels(context: android.content.Context) {
            ...
            // Progress/result text can include file names and command snippets from
            // whatever the agent is doing (see pushForegroundProgress in
            // ClaudeRuntimeBridge.kt). Keep that off the lock screen by default —
            // it's minor, but there's no reason to show it before the device unlocks.
