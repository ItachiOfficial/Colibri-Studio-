...
/**
 * One screen returned by Stitch for a design-preview flow. htmlCode is fetched and
 * cached lazily (only when the user taps "View HTML" or the screen is included in a
 * build) rather than eagerly for every generated screen — see StitchClient notes on
 * generation being slow/sometimes-unreliable; no need to pay that cost for screens
 * the user never looks at closely.
 */
data class StitchDesignScreen(
    val id: String,
    val title: String,
    val imageUrl: String?,
    val htmlUrl: String?,
...
enum class StitchPreviewStatus { IDLE, GENERATING, READY, FAILED }

data class StitchPreviewState(
    val status: StitchPreviewStatus = StitchPreviewStatus.IDLE,
    val prompt: String = "",
    val projectId: String? = null,
    val screens: List<StitchDesignScreen> = emptyList(),
    /** Order the user picked screens in — same ids as a subset of screens, preserves selection order for the flow arrow. */
    val selectedScreenIds: List<String> = emptyList(),
    val errorMessage: String? = null,
    val fullscreenScreenId: String? = null,
)
