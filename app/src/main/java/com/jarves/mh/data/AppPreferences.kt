...
class AppPreferences(private val context: Context) {
    ...
    /**
     * Whether HIGH-risk commands from Claude Code (rm -rf, sudo, git push, git
     * reset, curl — see classifyRisk() in ClaudeRuntimeBridge.kt) block on a real
     * approval before running. Defaults to true. Turning this off is an explicit,
     * conscious opt-out back to auto-allowing everything, not a hidden default —
     * see the Safety section in Settings. Read by RuntimeInstaller when it writes
     * permission-hook.sh, so a change here takes effect the next time the runtime
     * (re)installs the hook, not mid-session.
     */
    var confirmRiskyCommands: Boolean
        get() = preferences.getBoolean("confirm_risky_commands", true)
        set(value) { preferences.edit().putBoolean("confirm_risky_commands", value).apply() }
